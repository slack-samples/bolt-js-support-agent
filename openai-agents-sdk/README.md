...
```sh
slack create my-casey-agent --template slack-samples/bolt-js-support-agent --subdir openai-agents-sdk
cd my-casey-agent
```
...
cd my-casey-agent/openai-agents-sdk
```
...
The `support-agent.js` file defines the OpenAI Agents SDK Agent with a system prompt, personality, and tool configuration.
...
The `store.js` file implements an in-memory conversation history store, keyed by channel and thread. This enables multi-turn conversations where Casey remembers previous context within a thread. The store has TTL-based cleanup (1 hour) and a max entry limit (1000).
